        ...
        function visitEntity(hud, transform, vp, element) {
            ...
            if (!visible) {
                ...
                if (hud.getFlag(HeadsUpDisplayFlag.PerspectiveRotation)) {
                    ...
                    up.set(worldPositionX, worldPositionY + 1, worldPositionZ, 1);
                    up.threeApplyMatrix4(projectionMatrix);

                    const d = 1 / Math.abs(up.w);
                    up.x *= d;
                    up.y *= d;
                    up.z *= d;

                    const up_ndcX = (up.x + 1) / 2;
                    const up_ndcY = (1 - up.y) / 2;
