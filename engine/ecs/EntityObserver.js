...
function EntityObserver(componentTypes, completedCallback, brokenCallback, thisArg) {
    assert.ok(Array.isArray(componentTypes),'componentTypes must be an array, instead was something else');
    assert.typeOf(completedCallback, 'function','completedCallback');
    assert.typeOf(brokenCallback, 'function','brokenCallback');
