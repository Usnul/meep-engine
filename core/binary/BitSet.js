...
/**
 * Lookup table where each byte index is mapped to position of the first set bit
 * TODO consider bithacks: https://stackoverflow.com/questions/21623614/finding-the-first-set-bit-in-a-binary-number
 * http://graphics.stanford.edu/~seander/bithacks.html
 * @type {number[]}
 */
const FirstSetBitLookup = [8];
const FirstClearBitLookup = [0];
FirstClearBitLookup[255] = 8;

for (let i = 1; i < 256; i++) {
    for (let j = 0; j < 8; j++) {
        if ((MaskSet[j] & i) !== 0) {
            FirstSetBitLookup[i] = j;
            break;
        }
    }
...
BitSet.prototype.__setLength = function (newLength) {
    ...
    if (newLength > capacity) {
        ...
        if (newLength < capacity - RESIZE_COUNT_THRESHOLD && newLength < capacity * this.__shrinkFactor) {
...
BitSet.prototype.nextSetBit = function (fromIndex) {
    ...
    for (; byteIndex < byteLength; byteIndex++) {
        ...
        const bitIndex = FirstSetBitLookup[byte];
...
BitSet.prototype.nextClearBit = function (fromIndex) {
    ...
    for (; byteIndex < this.__length; byteIndex++) {
        ...
        const firstClearBit = FirstClearBitLookup[byte];
        if (firstClearBit < 8) {

            bitAddress = byteIndex * 8 + firstClearBit;
